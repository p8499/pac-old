    ...
    <div class="row">
        <ol class="breadcrumb">
            ...
            <li class="dropdown">
                ...
                <ul class="dropdown-menu" role="menu">
                    <li>
                        <a href="/fields?path=${pac:parent(requestScope.path)}.fields">
                            Fields</a>
                    </li>
                    <li>
                    ...
                    <li class="active">
                        <a href="/references?path=${requestScope.path}">
                            References</a>
                    </li>
                </ul>
            </li>
                ...
                <div class="panel-body">
                    <table class="table table-hover">
                        ...
                        <c:forEach items="${requestScope.target}" var="reference" varStatus="moduleStatus">
                            ...
                            <a href="/reference?path=${requestScope.path}[${moduleStatus.index}]">${pac:join(",",reference.domestic)}</a></span>
                                ...
                                <button type="button"
                                        ...
                                        onclick="$.get({url:'/uniques/swap?path=${requestScope.path}',data:{i:${moduleStatus.index-1},j:${moduleStatus.index}},success:function(response){window.location.reload();}});">
                                    <span class="glyphicon glyphicon-arrow-up"></span></button>
                                <button type="button"
                                        class="btn btn-default<c:if test="${moduleStatus.last}"> disabled</c:if>"
                                        onclick="$.get({url:'/uniques/swap?path=${requestScope.path}',data:{i:${moduleStatus.index},j:${moduleStatus.index+1}},success:function(response){window.location.reload();}});">
                                    <span class="glyphicon glyphicon-arrow-down"></span></button>
                                <button type="button" class="btn btn-danger"
                                        onclick="$.ajax({url:'/uniques/${moduleStatus.index}?path=${requestScope.path}',method:'DELETE',success:function(response){window.location.reload();}});">
    ...
    <div class="modal fade" id="post" aria-labelledby="post_label" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form onsubmit="event.preventDefault();$.ajax({url:'/references?path=${requestScope.path}',type:'POST',data:{domestic:$('#domestic').val(),foreignModule:$('#foreignModule').val(),foreign:$('#foreign').val()},success:function(response){window.location.reload();}});">

            ...
            <c:when test="${datasource.databaseType==\"postgresql\"}">
                <c:choose>
                    <c:when test="${pac:read(module,\"uniques[?(@.key)]\")[0].serial}">
                        @Insert("INSERT INTO ${module.databaseTable} (<c:forEach items="${pac:read(nonkeys,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">${pac:upper(field.databaseColumn)}<c:if test="${!fieldStatus.last}">,</c:if></c:forEach>) VALUES (<c:forEach items="${pac:read(nonkeys,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">${"#{"}bean.${field.databaseColumn},jdbcType=<c:choose><c:when test="${field.javaType==\"Integer\"}">${field.integerLength<5?"SMALLINT":field.integerLength<9?"INTEGER":"BIGINT"}</c:when><c:when test="${field.javaType==\"String\"}">VARCHAR</c:when><c:when test="${field.javaType==\"Double\"}">DOUBLE</c:when><c:when test="${field.javaType==\"java.util.Date\"}">TIMESTAMP</c:when></c:choose>${"}"}<c:if test="${!fieldStatus.last}">,</c:if></c:forEach>)")
                    ...
                    <c:otherwise>
                        @Insert("INSERT INTO ${module.databaseTable} (<c:forEach items="${pac:read(module.fields,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">${field.databaseColumn}<c:if test="${!fieldStatus.last}">,</c:if></c:forEach>) VALUES (<c:forEach items="${module.fields}" var="field" varStatus="fieldStatus">${"#{"}bean.${field.databaseColumn},jdbcType=<c:choose><c:when test="${field.javaType==\"Integer\"}">${field.integerLength<5?"SMALLINT":field.integerLength<9?"INTEGER":"BIGINT"}</c:when><c:when test="${field.javaType==\"String\"}">VARCHAR</c:when><c:when test="${field.javaType==\"Double\"}">DOUBLE</c:when><c:when test="${field.javaType==\"java.util.Date\"}">TIMESTAMP</c:when></c:choose>${"}"}<c:if test="${!fieldStatus.last}">,</c:if></c:forEach>)")</c:otherwise></c:choose></c:when>
            <c:when test="${datasource.databaseType==\"oracle\"}">
                <c:choose>
                    <c:when test="${pac:read(module,\"uniques[?(@.key)]\")[0].serial}">
                        @Insert("INSERT INTO ${module.databaseTable} (${pac:upper(pac:read(module,"uniques[?(@.key)]")[0].items[0])}<c:forEach items="${pac:read(nonkeys,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">,${pac:upper(field.databaseColumn)}</c:forEach>) VALUES (${module.databaseTable}_${pac:read(module,"uniques[?(@.key)]")[0].items[0]}.nextval<c:forEach items="${pac:read(nonkeys,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">,${"#{"}bean.${field.databaseColumn},jdbcType=<c:choose><c:when test="${field.javaType==\"Integer\"}">${field.integerLength<5?"SMALLINT":field.integerLength<9?"INTEGER":"BIGINT"}</c:when><c:when test="${field.javaType==\"String\"}">VARCHAR</c:when><c:when test="${field.javaType==\"Double\"}">DOUBLE</c:when><c:when test="${field.javaType==\"java.util.Date\"}">TIMESTAMP</c:when></c:choose>${"}"}</c:forEach>)")
                        @org.apache.ibatis.annotations.Options(useGeneratedKeys=true,keyProperty="bean.${pac:read(module,"uniques[?(@.key)]")[0].items[0]}")</c:when>
                    <c:otherwise>
                        @Insert("INSERT INTO ${module.databaseTable} (<c:forEach items="${pac:read(module.fields,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">${field.databaseColumn}<c:if test="${!fieldStatus.last}">,</c:if></c:forEach>) VALUES (<c:forEach items="${pac:read(module.fields,\"$.*[?(@.source=='table')]\")}" var="field" varStatus="fieldStatus">${"#{"}bean.${field.databaseColumn},jdbcType=<c:choose><c:when test="${field.javaType==\"Integer\"}">${field.integerLength<5?"SMALLINT":field.integerLength<9?"INTEGER":"BIGINT"}</c:when><c:when test="${field.javaType==\"String\"}">VARCHAR</c:when><c:when test="${field.javaType==\"Double\"}">DOUBLE</c:when><c:when test="${field.javaType==\"java.util.Date\"}">TIMESTAMP</c:when></c:choose>${"}"}<c:if test="${!fieldStatus.last}">,</c:if></c:forEach>)")</c:otherwise></c:choose></c:when>
        </c:choose>

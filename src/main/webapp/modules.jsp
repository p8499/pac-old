        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form data-toggle="validator"
                      onsubmit="event.preventDefault();$.ajax({url:'${baseUrl}modules?path=${requestScope.path}',type:'POST',data:{id:$('#id').val(),description:$('#description').val(),comment:$('#comment').val(),datasource:$('#datasource').val(),databaseTable:$('#databaseTable').val(),databaseView:$('#databaseView').val(),jteeBeanAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1),jteeMaskAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1)+'Mask',jteeMapperAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1)+'Mapper',jteeControllerBaseAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1)+'ControllerBase',jteeControllerPath:'api/'+$('#id').val()+'/',jteeAttachmentControllerPath:'api/'+$('#id').val()+'_attachment/',androidBeanAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1),androidMaskAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1)+'Mask',androidStubAlias:$('#id').val().substr(0,1).toUpperCase()+$('#id').val().substr(1)+'Stub'},success:function(response){window.location.reload();}});">

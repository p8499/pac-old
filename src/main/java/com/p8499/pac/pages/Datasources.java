...
@Controller
@RequestMapping(value = "/datasources", produces = "application/json;charset=UTF-8")
public class Datasources {
    ...
    public String add(HttpSession session, HttpServletRequest request, HttpServletResponse response, @RequestParam(required = true) String path, @RequestBody String body) throws UnsupportedEncodingException {
        ...
        model.put("databaseType", Util.extract(body, "databaseType"));
        model.put("url", Util.extract(body, "url"));
        model.put("username", Util.extract(body, "username"));
        model.put("password", Util.extract(body, "password"));
        target.add(model);
        return "";
    }

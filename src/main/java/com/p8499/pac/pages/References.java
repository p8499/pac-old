...
@Controller
@RequestMapping(value = "/references", produces = "application/json;charset=UTF-8")
public class References {
    ...
    public String add(HttpSession session, HttpServletRequest request, HttpServletResponse response, @RequestParam(required = true) String path, @RequestBody String body) throws UnsupportedEncodingException {
        List target = (List) Util.read(request.getSession().getAttribute("json"), path);
        Map model = (Map) Configuration.defaultConfiguration().jsonProvider().parse("{\"domestic\":[],\"foreign\":[]}");
        model.put("domestic", Configuration.defaultConfiguration().jsonProvider().parse(Util.wrap(((String) Util.extract(body, "domestic")).split(","))));
        model.put("foreignModule", Util.extract(body, "foreignModule"));
        model.put("foreign", Configuration.defaultConfiguration().jsonProvider().parse(Util.wrap(((String) Util.extract(body, "foreign")).split(","))));
        target.add(model);
        return "";
    }
